...
<head>
    ...
    $(document).ready(function(){ 
    	...
    	$('#active').click(function(){
    		...
    		var temp = $(".board_table > tbody > tr > td:nth-child(6n+3):contains('게시')");
    		$(temp).parent().show();
    	});   	
...
<body>
	<% 
		...
		ArrayList<Bbs_faq> list = new BbsDAO_faq().getFAQ();

		if(value.equals("")==false){
			//검색어가 있는 경우
			list = new BbsDAO_faq().getFAQ(option, value);
		}
	%>
        ...
        <section>		
    		<div class="board_container"> 
	    		...
	    		<div class="board_search">	
	    			<form method="post" action="admin_bbsFaq.jsp">    			
            	...
            	<div class="board_page-move">
            		...
            		<div class="board_page-move-symbol-left">
            			<a href="admin_bbsFaq.jsp?pageNumber=<%=pageNumber-1 %>&value=<%=value %>&option=<%=option %>" class="link"> ◀ 이전 페이지 </a>
					</div>
					<div class="board_page-move-symbol-right">
            			<a href="admin_bbsFaq.jsp?pageNumber=<%=pageNumber+1 %>&value=<%=value %>&option=<%=option %>" class="link"> 다음 페이지 ▶ </a>
            		</div>
            	</div>
            </div>   		
    	</section>
